...
module.exports = HandleMsg = async (urbae, message) => {
	try {
		...
		// [IDENTIFY]
		const ownerNumber = "6281385351530@c.us"
		...
		if (chats == 'kontol' || chats == 'Kontol') {
			urbae.reply(from, 'Jangan Jorok kau Asu ', id)
		}
		if (chats == 'sayang') {
			urbae.reply(from, 'Iya Sayang Kenapa', id)
		}
		if (chats == 'bot'){
			axios.get(`https://api.simsimi.net/v2/?text=${chats}&lc=id`)//if you wamt to change language, change "lc"= "country code"
			.then(res => {
		...
		const mess = {
			...
			prem: `Premium only, Makanya bayar`,
			error: {
		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				...
				case prefix + 'tnc':
					...
					break
				case prefix + 'help':
					const bots = `Hi ${pushname}, this is Deadrz Bot, to find out the commands menu, type *${prefix}menu* , *${prefix}p*`
				...
				case prefix + 'citacita'://Piyobot
					...
					break
				case prefix + 'tgljodoh':
					if (args.length == 0 || args.length == 1 || args.length == 2) return urbae.reply(from, `Untuk mengetahui ramalan tanggal Jadian & Nikah kamu \nketik: ${prefix}tgljodoh hari-ke bulan tahun\nContoh : ${prefix}tgljodoh 06 01 2018`, id)
					let tgl = args[0]+"-"+args[1]+"-"+args[2]
					fetchJson(`https://api.akuari.my.id/primbon/tanggaljadian?tanggal=${tgl}`)
						.then(async (res) => {
							urbae.reply(from, res.hasil, id)
								.catch(err => {
									console.log(err)
				...
				case prefix + 'blackpink':
					...
					urbae.sendFileFromUrl(from, `https://api.akuari.my.id/ephoto/blackpink?text=${bpk}`, `${bpk}.jpg`, `nehh ngab`, id)
						.catch(err => {
							console.log(err)
							urbae.reply(from, err.message, id)
						})
				...
				case prefix + 'logoph':
					...
					if (args.length === 1) return urbae.reply(from, `Kirim perintah *${prefix}logoph  Teks1 Teks2 ]*,\n\n contoh : *${prefix}logoph Urbaee Xyz*`, id)
					...
					urbae.sendFileFromUrl(from, `https://api.akuari.my.id/ephoto/pornhub?text=${lpornhub}&text_2=${lpornhub2}`, 'as.jpg', 'Noh', id)
						.catch(err => {
							console.log(err)
							urbae.reply(from, 'Error Ngab', id)
						})
					...
					if (isPrem) {
						await urbae.joinGroupViaLink(linkgrup)
							.then(async () => {
								...
								await urbae.sendText(chekgrup.id, `what up y'all , I'm Deadrz Bot. To find out the commands on this Bot type ${prefix}menu`)
							})
					} else {
				...
				case prefix + 'doaharian':
					fetchJson('https://api.akuari.my.id/islami/doa')
						.then(async (res) => {
							let harian = `*「 DOA HARIAN 」*`
							harian += `\n─────────────────\n\n• *Judul Doa:* ${res.result.title}\n• *Arab:* ${res.result.arabic}\n• *Latin:* ${res.result.latin}\n• *Arti:* ${res.result.translation}\n\n`
							urbae.reply(from, harian, id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, 'Sedang error', id)
								})
				...
				break*/
				case prefix+'ghea':
					axios.get('http://202.157.176.247:8000/api.php?id=https://api.akuari.my.id/asupan/ghea')
					.then(async(res) => {
						urbae.sendFileFromUrl(from, res.data, 'ghea.mp4', 'nih asupann ghea', id)
						.catch(() => {
							urbae.reply(from, 'Error, mungkin server api sedang down', id)
						})
					})
					...
					break
					case prefix+'bocil':
					axios.get('https://api.akuari.my.id/asupan/bocil')
					.then(async(res) => {
						urbae.sendFileFromUrl(from, res.data.respon, 'bocil.mp4', 'Nih asupann Bocil', id)
						.catch(() => {
							urbae.reply(from, 'Error, mungkin server api sedang down', id)
						})
					})
					...
					break
					case prefix+'ukhty':
					axios.get('https://api.akuari.my.id/asupan/ukhty')
					.then(async(res) => {
						urbae.sendFileFromUrl(from, res.data.respon, 'ukhty.mp4', 'Nih asupann Ukhty', id)
						.catch(() => {
							urbae.reply(from, 'Error, mungkin server api sedang down', id)
						})
					})
					...
					break
					case prefix+'santuy':
					axios.get('https://api.akuari.my.id/asupan/santuy')
					.then(async(res) => {
						urbae.sendFileFromUrl(from, res.data.respon, 'santuy.mp4', 'Nih asupann Santuy', id)
						.catch(() => {
							urbae.reply(from, 'Error, mungkin server api sedang down', id)
						})
					})
				...
				case prefix + 'sindiran':
					axios.get(`https://api.akuari.my.id/randomtext/sindiran`)
						.then(async (res) => {
							urbae.reply(from, res.hasil.result, id)
								.catch(() => {
				...
				case prefix + 'artinama':
					...
					if(args[0] && args[1] && args[2] && args[3]){
						let kon = args[0]+" "+args[1]+" "+args[2]+" "+args[3]
						axios.get(`https://api.akuari.my.id/primbon/artinama?nama=${kon}`)
						.then(async (res) => {
							await urbae.reply(from, res.data, id)
								.catch(() => {
									urbae.reply(from, 'Sedang error', id)
								})
						})
				...
				case prefix + 'cekjodoh':
					if (args.length == 0) return urbae.reply(from, `Untuk mengecek jodoh melalui nama\nketik: ${prefix}cekjodoh nama-panggilan-kamu nama-panggilan-pasangan\n\ncontoh: ${prefix}cekjodoh bagas siti\n\nhanya bisa pakai nama panggilan (satu kata)`, id)
					let km = args[0]
					let dia = args[1]
					axios.get(`https://api.akuari.my.id/primbon/ramalanjodoh?nama=${km}&pasangan=${dia}`)
						.then(async (res) => {
							urbae.reply(from, `${km} 💘 ${dia}  \n\nPositif : ${res.data.hasil.positif} \nNegatif : ${res.data.hasil.negatif}`, id)
							console.log(res)
						})
					break
				...
				case prefix + 'katabijak':
					fetchJson('https://api.akuari.my.id/randomtext/katabijak')
						.then(async (res) => {
							urbae.reply(from, `*Author : ${res.hasil.author} 😎*\n\n${res.hasil.quotes}`, id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, 'Sedang error', id)
								})
					...
					break
				case prefix + 'katadedi':
					fetchJson('https://api.akuari.my.id/randomtext/katailham')
						.then(async (res) => {
							urbae.reply(from, `👉 ${res.hasil.result}\n😎    🤪`, id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, 'Sedang error', id)
								})
					...
					break
				case prefix + 'fakboy':
					fetchJson('https://api.akuari.my.id/randomtext/pantunpakboy')
						.then(async (res) => {
							urbae.reply(from, `👇\n${res.hasil.result} 😎`, id)
								.catch(err => {
									console.log(err)
						...
						yt.ytSearch(body.slice(10))
							.then(async (res) => {
								...
								hxzapi.youtube(res[0].url)
									.then(async (result) => {
										if (Number(result.size_mp3.split(' MB')[0]) >= 9) return urbae.reply(from, `Size audio terlalu besar Kontol\nSilahkan download manual menggunakan link dibawah\nLink: ${result.mp3}`, id)
							...
							.then(async (res) => {
								...
								hxzapi.youtube(res[0].url)
									.then(async (result) => {
										...
										.catch(err => {
											console.log(err)
											urbae.reply(from, 'Lagu aja anjing', id)
										})
									})
							})
					}
